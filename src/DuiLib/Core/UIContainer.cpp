...
namespace DuiLib {
    ...
    void CContainerUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue) {
        if( _tcsicmp(pstrName, _T("inset")) == 0 ) {
            ...
            if( GetVerticalScrollBar() ) {
                LPCTSTR pStyle = m_pManager->GetStyle(m_sVerticalScrollBarStyle.GetDataPointer());
            ...
            if( GetHorizontalScrollBar() ) {
                LPCTSTR pStyle = m_pManager->GetStyle(m_sHorizontalScrollBarStyle.GetDataPointer());
    ...
    base::String CContainerUI::GetSubControlText( LPCTSTR pstrSubControlName ) {
        CControlUI *pSubControl = NULL;
        pSubControl = this->FindSubControl(pstrSubControlName);
    ...
    const base::String CContainerUI::GetSubControlUserData( LPCTSTR pstrSubControlName ) {
        CControlUI *pSubControl = NULL;
        pSubControl = this->FindSubControl(pstrSubControlName);
