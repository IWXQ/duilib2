...
namespace DuiLib {
    ...
    bool DrawImage(HDC hDC, CPaintManagerUI *pManager, const RECT &rc, const RECT &rcPaint, const base::String &sImageName, \
                   const base::String &sImageResType, RECT rcItem, RECT rcBmpPart, RECT rcCorner, DWORD dwMask, BYTE bFade, \
        ...
        if( sImageResType.IsEmpty() ) {
            data = pManager->GetImageEx((LPCTSTR)sImageName.GetDataPointer(), NULL, dwMask, false, instance);
        } else {
            data = pManager->GetImageEx((LPCTSTR)sImageName.GetDataPointer(), (LPCTSTR)sImageResType.GetDataPointer(), dwMask, false, instance);
        }
        ...
        do {
            if( type == NULL ) {
                ...
                if( CPaintManagerUI::GetResourceZip().IsEmpty() ) {
                    ...
                    ZIPENTRY ze;
                    int i = 0;
                    base::String key = bitmap.m_lpstr;
                    key.Replace(_T("\\"), _T("/"));
    ...
    TImageInfo *CRenderEngine::LoadImage(LPCTSTR pStrImage, LPCTSTR type, DWORD mask, HINSTANCE instance) {
        ...
        base::String sStrPath = pStrImage;
        ...
        return LoadImage(STRINGorID(sStrPath.GetDataPointer()), type, mask, instance);
    }
    ...
    void CRenderEngine::DrawText(HDC hDC, CPaintManagerUI *pManager, RECT &rc, LPCTSTR pstrText, DWORD dwTextColor, int iFont, UINT uStyle) {
        ...
        if ( pManager->IsLayered() || pManager->IsUseGdiplusText()) {
            ...
            HFONT hOldFont = (HFONT)::SelectObject(hDC, pManager->GetFont(iFont));
            int fonticonpos = base::String(pstrText).Find(_T("&#x"));

            if (fonticonpos != -1) {
                base::String strUnicode = base::String(pstrText).Mid(fonticonpos + 3);
                ...
                wchar_t wch[2] = { 0 };
                wch[0] = static_cast<wchar_t>(_tcstol(strUnicode.GetDataPointer(), 0, 16));
    ...
    void CRenderEngine::DrawHtmlText(HDC hDC, CPaintManagerUI *pManager, RECT &rc, LPCTSTR pstrText, DWORD dwTextColor, RECT *prcLinks, base::String *sLinks, int &nLinkRects, int iFont, UINT uStyle) {
            ...
            if( pt.x >= rc.right || *pstrText == _T('\n') || bLineEnd ) {
                ...
                if( !bLineDraw ) {
                    if( bInLink && iLinkIndex < nLinkRects ) {
                        ...
                        base::String *pStr1 = (base::String *)(sLinks + iLinkIndex - 1);
                        base::String *pStr2 = (base::String *)(sLinks + iLinkIndex);
                        *pStr2 = *pStr1;
                    }
                ...
                switch( *pstrText ) {
                    case _T('a'): { // Link
                            ...
                            if( iLinkIndex < nLinkRects && !bLineDraw ) {
                                base::String *pStr = (base::String *)(sLinks + iLinkIndex);
                                pStr->Empty();
                            ...
                            if( pFontInfo->bUnderline == false ) {
                                HFONT hFont = pManager->GetFont(pFontInfo->sFontName.GetDataPointer(), pFontInfo->iSize, pFontInfo->bBold, true, pFontInfo->bItalic);

                                if( hFont == NULL ) hFont = pManager->AddFont(g_iFontID, pFontInfo->sFontName.GetDataPointer(), pFontInfo->iSize, pFontInfo->bBold, true, pFontInfo->bItalic);
                    ...
                    case _T('b'): { // Bold
                            ...
                            if( pFontInfo->bBold == false ) {
                                HFONT hFont = pManager->GetFont(pFontInfo->sFontName.GetDataPointer(), pFontInfo->iSize, true, pFontInfo->bUnderline, pFontInfo->bItalic);

                                if( hFont == NULL ) hFont = pManager->AddFont(g_iFontID, pFontInfo->sFontName.GetDataPointer(), pFontInfo->iSize, true, pFontInfo->bUnderline, pFontInfo->bItalic);
                    ...
                    case _T('f'): { // Font
                            ...
                            if( pstrTemp != pstrText ) {
                                ...
                                HFONT hFont = pManager->GetFont(sFontName.GetDataPointer(), iFontSize, bBold, bUnderline, bItalic);

                                if( hFont == NULL ) hFont = pManager->AddFont(g_iFontID, sFontName.GetDataPointer(), iFontSize, bBold, bUnderline, bItalic);
                    ...
                    case _T('i'): { // Italic or Image
                            ...
                            pstrText++;
                            base::String sImageString = pstrText;
                            int iWidth = 0;
                            int iHeight = 0;
                            ...
                            if( sName.IsEmpty() ) { // Italic
                                ...
                                if( pFontInfo->bItalic == false ) {
                                    HFONT hFont = pManager->GetFont(pFontInfo->sFontName.GetDataPointer(), pFontInfo->iSize, pFontInfo->bBold, pFontInfo->bUnderline, true);

                                    if( hFont == NULL ) hFont = pManager->AddFont(g_iFontID, pFontInfo->sFontName.GetDataPointer(), pFontInfo->iSize, pFontInfo->bBold, pFontInfo->bUnderline, true);
                                ...
                                if( _tcsstr(sImageString.GetDataPointer(), _T("file=\'")) != NULL || _tcsstr(sImageString.GetDataPointer(), _T("res=\'")) != NULL ) {
                                    base::String sImageResType;
                                    base::String sImageName;
                                    LPCTSTR pStrImage = sImageString.GetDataPointer();
                                    base::String sItem;
                                    base::String sValue;
                                    ...
                                    pImageInfo = pManager->GetImageEx((LPCTSTR)sImageName.GetDataPointer(), sImageResType.GetDataPointer());
                                } else
                                    pImageInfo = pManager->GetImageEx((LPCTSTR)sName.GetDataPointer());
