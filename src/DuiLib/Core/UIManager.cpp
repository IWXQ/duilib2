...
namespace DuiLib {
            ...
            while( *pStrImage != _T('\0') ) {
                ...
                if( !sValue.IsEmpty() ) {
                    if( sItem == _T("file") || sItem == _T("res") ) {
                        ...
                        rcDest.left = _tcstol(sValue.GetDataPointer(), &pstr, 10);
                        ASSERT(pstr);
                        ...
                        rcSource.left = _tcstol(sValue.GetDataPointer(), &pstr, 10);
                        ASSERT(pstr);
                        ...
                        rcCorner.left = _tcstol(sValue.GetDataPointer(), &pstr, 10);
                        ASSERT(pstr);
                        ...
                        if( sValue[0] == _T('#')) 
                            dwMask = _tcstoul(sValue.GetDataPointer() + 1, &pstr, 16);
                        else 
                            dwMask = _tcstoul(sValue.GetDataPointer(), &pstr, 16);
                    } 
                    else if( sItem == _T("fade") ) {
                        uFade = (BYTE)_tcstoul(sValue.GetDataPointer(), &pstr, 10);
                    } 
                    else if( sItem == _T("hole") ) {
                        bHole = (_tcsicmp(sValue.GetDataPointer(), _T("true")) == 0);
                    } 
                    else if( sItem == _T("xtiled") ) {
                        bTiledX = (_tcsicmp(sValue.GetDataPointer(), _T("true")) == 0);
                    } 
                    else if( sItem == _T("ytiled") ) {
                        bTiledY = (_tcsicmp(sValue.GetDataPointer(), _T("true")) == 0);
                    }
                    else if( sItem == _T("hsl") ) {
                        bHSL = (_tcsicmp(sValue.GetDataPointer(), _T("true")) == 0);
                    }
                    // Jeffery: 是否适应DPI缩放
                    //
                    else if (sItem == _T("adaptdpiscale")) {
                        bAdaptDpiScale = (_tcsicmp(sValue.GetDataPointer(), _T("true")) == 0);
                    }
                }
        ...
        if (bAdaptDpiScale && pManager->GetDPIObj()->GetScale() != 100) {
            ...
            do {
                if (sResType.GetLength() == 0) {
                    base::String sFile = CPaintManagerUI::GetResourcePath();

                    if (CPaintManagerUI::GetResourceZip().IsEmpty()) {
                        sFile += newImageName;
                        HANDLE hFile = ::CreateFile(sFile.GetDataPointer(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
                        ...
                        base::String sFilePwd = CPaintManagerUI::GetResourceZipPwd();
                        HZIP hz = NULL;
...
#ifdef UNICODE
                            std::string pwd = base::UnicodeToAnsi(sFilePwd.GetDataPointer());
                            hz = OpenZip(sFile.GetDataPointer(), pwd.c_str());
#else
                            hz = OpenZip(sFile.GetDataPointer(), sFilePwd.GetDataPointer());
#endif
                        }
                        ...
                        ZIPENTRY ze;
                        int i = 0;
                        base::String key = newImageName;
                        key.Replace(_T("\\"), _T("/"));

                        if (FindZipItem(hz, key.GetDataPointer(), true, &i, &ze) != 0) {
                            if (!CPaintManagerUI::IsCachedResourceZip())
                                CloseZip(hz);
                            break;
                        }
                    ...
                    HRSRC hResource = ::FindResource(dllinstance, newImageName.GetDataPointer(), sResType.GetDataPointer());
                    if (hResource == NULL)
                        break;
    ...
    base::String CPaintManagerUI::m_pStrResourcePath;
    base::String CPaintManagerUI::m_pStrResourceZip;
    base::String CPaintManagerUI::m_pStrResourceZipPwd;  //Garfield 20160325 带密码zip包解密
    ...
    base::String CPaintManagerUI::GetCurrentPath() {
        TCHAR tszModule[MAX_PATH + 1] = { 0 };
    ...
    const base::String &CPaintManagerUI::GetResourcePath() {
        return m_pStrResourcePath;
    }

    const base::String &CPaintManagerUI::GetResourceZip() {
        return m_pStrResourceZip;
    }

    const base::String &CPaintManagerUI::GetResourceZipPwd() {
    ...
    void CPaintManagerUI::SetResourceZip(LPVOID pVoid, unsigned int len, LPCTSTR password) {
        ...
        if( m_bCachedResourceZip ) {
            ...
            if(password)
                pwd = base::UnicodeToAnsi(password);
    ...
    void CPaintManagerUI::SetResourceZip(LPCTSTR pStrPath, bool bCachedResourceZip, LPCTSTR password) {
        ...
        if( m_bCachedResourceZip ) {
            ...
            if(password)
                pwd = base::UnicodeToAnsi((wchar_t *)password);

            m_hResourceZip = (HANDLE)OpenZip(sFile.GetDataPointer(), pwd.c_str());
#else
            m_hResourceZip = (HANDLE)OpenZip(sFile.GetDataPointer(), password);
#endif
        }
    }

    void CPaintManagerUI::SetResourceZip(int resourceID, const base::String &strResType, LPCTSTR password /*= NULL*/) {
        HRSRC hResource = ::FindResource(CPaintManagerUI::GetResourceDll(), MAKEINTRESOURCE(resourceID), strResType.GetDataPointer());
    ...
    LPCTSTR CPaintManagerUI::GetLayeredImage() {
        return m_diLayered.sDrawString.GetDataPointer();
    }
    ...
    bool CPaintManagerUI::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT &lRes) {
        ...
        switch( uMsg ) {
            ...
            case WM_MOUSEHOVER: {
                    ...
                    m_ToolTip.lpszText = const_cast<LPTSTR>( (LPCTSTR) sToolTip.GetDataPointer());
    ...
    void CPaintManagerUI::ReapObjects(CControlUI *pControl) {
        ...
        KillTimer(pControl);
        const base::String &sName = pControl->GetName();

        if( !sName.IsEmpty() ) {
            if( pControl == FindControl(sName.GetDataPointer()) ) m_mNameHash.Remove(sName.GetDataPointer());
        }
    ...
    void CPaintManagerUI::RemoveAllOptionGroups() {
		...
		for (std::map<base::String, LPVOID>::iterator it = m_mOptionGroup.Begin(); it != m_mOptionGroup.End(); it++) {
    ...
    void CPaintManagerUI::MessageLoop() {
        ...
        while( ::GetMessage(&msg, NULL, 0, 0) ) {
            if (msg.hwnd == NULL && msg.message == UIMSG_THREADMSG && msg.lParam == UIMSG_THREADMSG) { // Thread Message
                base::Closure *p = (base::Closure*)(msg.wParam);
                if (p) {
                    if(!p->is_null())
                        p->Run();
    ...
    void CPaintManagerUI::Term() {
		...
		for (std::map<base::String, LPVOID>::iterator it = m_SharedResInfo.m_ImageHash.Begin(); it != m_SharedResInfo.m_ImageHash.End(); it++) {
		...
		for (std::map<base::String, LPVOID>::iterator it = m_SharedResInfo.m_CustomFonts.Begin(); it != m_SharedResInfo.m_CustomFonts.End(); it++) {
        ...
        // 样式
        base::String *pStyle = NULL;
		for (std::map<base::String, LPVOID>::iterator it = m_SharedResInfo.m_StyleHash.Begin(); it != m_SharedResInfo.m_StyleHash.End(); it++) {
			pStyle = static_cast<base::String *>(it->second);
        ...
        // 样式
        base::String *pAttr = NULL;
		for (std::map<base::String, LPVOID>::iterator it = m_SharedResInfo.m_AttrHash.Begin(); it != m_SharedResInfo.m_AttrHash.End(); it++) {
			pAttr = static_cast<base::String *>(it->second);
    ...
    void DuiLib::CPaintManagerUI::ResetDPIAssets() {
		...
		for (std::map<base::String, LPVOID>::iterator it = m_ResInfo.m_CustomFonts.Begin(); it != m_ResInfo.m_CustomFonts.End(); it++) {
    ...
    void DuiLib::CPaintManagerUI::RebuildFont(TFontInfo *pFontInfo) {
        ...
        LOGFONT lf = { 0 };
        ::GetObject(::GetStockObject(DEFAULT_GUI_FONT), sizeof(LOGFONT), &lf);
        _tcsncpy(lf.lfFaceName, pFontInfo->sFontName.GetDataPointer(), LF_FACESIZE);
        lf.lfCharSet = DEFAULT_CHARSET;
    ...
    void CPaintManagerUI::AddFontArray(LPCTSTR pstrPath) {
        ...
        do {
            ...
            if (CPaintManagerUI::GetResourceZip().IsEmpty()) {
                sFile += pstrPath;
                HANDLE hFile = ::CreateFile(sFile.GetDataPointer(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, \
                ...
                if (CPaintManagerUI::IsCachedResourceZip()) hz = (HZIP)CPaintManagerUI::GetResourceZipHandle();
                    ...
                    base::String sFilePwd = CPaintManagerUI::GetResourceZipPwd();
#ifdef UNICODE
                    std::string pwd = base::UnicodeToAnsi((wchar_t *)sFilePwd.GetDataPointer());
                    hz = OpenZip(sFile.GetDataPointer(), pwd.c_str());
#else
                    hz = OpenZip(sFile.GetDataPointer(), sFilePwd.GetDataPointer());
#endif
                ...
                ZIPENTRY ze;
                int i = 0;
                base::String key = pstrPath;
                key.Replace(_T("\\"), _T("/"));

                if (FindZipItem(hz, key.GetDataPointer(), true, &i, &ze) != 0) break;
        ...
        if (bShared) {
			for (std::map<base::String, LPVOID>::iterator it = m_SharedResInfo.m_CustomFonts.Begin(); it != m_SharedResInfo.m_CustomFonts.End(); it++) {
				...
				if (pFontInfo && pFontInfo->hFont == hFont) 
					return _ttoi(it->first.GetDataPointer());
			}
        } else {
			for (std::map<base::String, LPVOID>::iterator it = m_ResInfo.m_CustomFonts.Begin(); it != m_ResInfo.m_CustomFonts.End(); it++) {
				pFontInfo = static_cast<TFontInfo *>(it->second);
        ...
        if (bShared) {
			for (std::map<base::String, LPVOID>::iterator it = m_SharedResInfo.m_CustomFonts.Begin(); it != m_SharedResInfo.m_CustomFonts.End(); it++) {
				...
				if (pFontInfo && pFontInfo->hFont == hFont) {
					::DeleteObject(pFontInfo->hFont);
					delete pFontInfo;
					m_SharedResInfo.m_CustomFonts.Remove(it->first.GetDataPointer());
					return;
				}
			}
        } else {
			for (std::map<base::String, LPVOID>::iterator it = m_ResInfo.m_CustomFonts.Begin(); it != m_ResInfo.m_CustomFonts.End(); it++) {
				...
				if (pFontInfo && pFontInfo->hFont == hFont) {
					::DeleteObject(pFontInfo->hFont);
					delete pFontInfo;
					m_ResInfo.m_CustomFonts.Remove(it->first.GetDataPointer());
					return;
				}
			}
        }
    ...
    void CPaintManagerUI::RemoveAllImages(bool bShared) {
        if (bShared) {
            ...
            TImageInfo *data = NULL;
			for (std::map<base::String, LPVOID>::iterator it = m_ResInfo.m_ImageHash.Begin(); it != m_ResInfo.m_ImageHash.End(); it++) {
				data = static_cast<TImageInfo *>(it->second);
				if (data) {
					CRenderEngine::FreeImage(data);
				}
		...
		for (std::map<base::String, LPVOID>::iterator it = m_SharedResInfo.m_ImageHash.Begin(); it != m_SharedResInfo.m_ImageHash.End(); it++) {
			...
			if (data != NULL) {
				if (!data->sResType.IsEmpty()) {
					if (isdigit(*(it->first.GetDataPointer()))) {
						LPTSTR pstr = NULL;
						int iIndex = _tcstol(it->first.GetDataPointer(), &pstr, 10);
						pNewData = CRenderEngine::LoadImage(iIndex, data->sResType.GetDataPointer(), data->dwMask);
					}
				}
				else {
					pNewData = CRenderEngine::LoadImage(it->first.GetDataPointer(), NULL, data->dwMask);
				}
    ...
    void CPaintManagerUI::ReloadImages() {
		...
		for (std::map<base::String, LPVOID>::iterator it = m_ResInfo.m_ImageHash.Begin(); it != m_ResInfo.m_ImageHash.End(); it++) {
			...
			if (data != NULL) {
				...
				if (pNewData == NULL) {
					m_ResInfo.m_ImageHash.Remove(it->first.GetDataPointer());
					continue;
				}
    ...
    const TDrawInfo *CPaintManagerUI::GetDrawInfo(LPCTSTR pStrImage, LPCTSTR pStrModify) {
        base::String sStrImage = pStrImage;
        base::String sStrModify = pStrModify;
        base::String sKey = sStrImage + sStrModify;
        TDrawInfo *pDrawInfo = static_cast<TDrawInfo *>(m_ResInfo.m_DrawInfoHash.Find(sKey.GetDataPointer()));

        if(pDrawInfo == NULL && !sKey.IsEmpty()) {
            pDrawInfo = new TDrawInfo();
            pDrawInfo->Parse(pStrImage, pStrModify, this);
            m_ResInfo.m_DrawInfoHash.Insert(sKey.GetDataPointer(), pDrawInfo);
        }
    ...
    void CPaintManagerUI::RemoveDrawInfo(LPCTSTR pStrImage, LPCTSTR pStrModify) {
        ...
        if(pDrawInfo != NULL) {
            m_ResInfo.m_DrawInfoHash.Remove(sKey.GetDataPointer());
            delete pDrawInfo;
            pDrawInfo = NULL;
        }
    }
    ...
    void CPaintManagerUI::RemoveAllDrawInfos() {
		...
		for (std::map<base::String, LPVOID>::iterator it = m_ResInfo.m_DrawInfoHash.Begin(); it != m_ResInfo.m_DrawInfoHash.End(); it++) {
    ...
    void CPaintManagerUI::AddDefaultAttributeList(LPCTSTR pStrControlName, LPCTSTR pStrControlAttrList, bool bShared) {
        if (bShared || m_bForceUseSharedRes) {
            base::String *pDefaultAttr = new base::String(pStrControlAttrList);

            if (pDefaultAttr != NULL) {
                base::String *pOldDefaultAttr = static_cast<base::String *>(m_SharedResInfo.m_AttrHash.Set(pStrControlName, (LPVOID)pDefaultAttr));
            ...
            if (pDefaultAttr != NULL) {
                base::String *pOldDefaultAttr = static_cast<base::String *>(m_ResInfo.m_AttrHash.Set(pStrControlName, (LPVOID)pDefaultAttr));
    ...
    LPCTSTR CPaintManagerUI::GetDefaultAttributeList(LPCTSTR pStrControlName) const {
        base::String *pDefaultAttr = static_cast<base::String *>(m_ResInfo.m_AttrHash.Find(pStrControlName));

        if( !pDefaultAttr )
            pDefaultAttr = static_cast<base::String *>(m_SharedResInfo.m_AttrHash.Find(pStrControlName));

        if (pDefaultAttr) 
            return pDefaultAttr->GetDataPointer();
    ...
    bool CPaintManagerUI::RemoveDefaultAttributeList(LPCTSTR pStrControlName, bool bShared) {
        if (bShared) {
            base::String *pDefaultAttr = static_cast<base::String *>(m_SharedResInfo.m_AttrHash.Find(pStrControlName));
    ...
    void CPaintManagerUI::RemoveAllDefaultAttributeList(bool bShared) {
        if (bShared) {
            base::String *pDefaultAttr;
			for (std::map<base::String, LPVOID>::iterator it = m_SharedResInfo.m_AttrHash.Begin(); it != m_SharedResInfo.m_AttrHash.End(); it++) {
				pDefaultAttr = static_cast<base::String *>(it->second);
            ...
            base::String *pDefaultAttr;
			for (std::map<base::String, LPVOID>::iterator it = m_ResInfo.m_AttrHash.Begin(); it != m_ResInfo.m_AttrHash.End(); it++) {
				pDefaultAttr = static_cast<base::String *>(it->second);
    ...
    void CPaintManagerUI::AddWindowCustomAttribute(LPCTSTR pstrName, LPCTSTR pstrAttr) {
        ...
        base::String *pCostomAttr = new base::String(pstrAttr);
    ...
    LPCTSTR CPaintManagerUI::GetWindowCustomAttribute(LPCTSTR pstrName) const {
        ...
        base::String *pCostomAttr = static_cast<base::String *>(m_mWindowCustomAttrHash.Find(pstrName));

        if( pCostomAttr ) 
            return pCostomAttr->GetDataPointer();
    ...
    void CPaintManagerUI::RemoveAllWindowCustomAttribute() {
        base::String *pCostomAttr;
		for (std::map<base::String, LPVOID>::iterator it = m_mWindowCustomAttrHash.Begin(); it != m_mWindowCustomAttrHash.End(); it++) {
			pCostomAttr = static_cast<base::String *>(it->second);
			if(pCostomAttr)
				delete pCostomAttr;
		}
    ...
    CControlUI *CALLBACK CPaintManagerUI::__FindControlFromNameHash(CControlUI *pThis, LPVOID pData) {
        ...
        pManager->m_mNameHash.Set(sName.GetDataPointer(), pThis);
    ...
    CControlUI *CALLBACK CPaintManagerUI::__FindControlFromName(CControlUI *pThis, LPVOID pData) {
        ...
        base::String sName = pThis->GetName();

        if( sName.IsEmpty() ) 
            return NULL;

        return (_tcsicmp(sName.GetDataPointer(), pstrName) == 0) ? pThis : NULL;
    }
    ...
    void CPaintManagerUI::AddStyle(LPCTSTR pName, LPCTSTR pDeclarationList, bool bShared) {
        base::String *pStyle = new base::String(pDeclarationList);
    ...
    LPCTSTR CPaintManagerUI::GetStyle(LPCTSTR pName) const {
        base::String *pStyle = static_cast<base::String *>(m_ResInfo.m_StyleHash.Find(pName));

        if( !pStyle )
            pStyle = static_cast<base::String *>(m_SharedResInfo.m_StyleHash.Find(pName));

        if( pStyle ) 
            return pStyle->GetDataPointer();
        else return NULL;
    }
    ...
    BOOL CPaintManagerUI::RemoveStyle(LPCTSTR pName, bool bShared) {
        ...
        if (bShared) {
            ...
            pStyle = static_cast<base::String *>(m_ResInfo.m_StyleHash.Find(pName));
    ...
    void CPaintManagerUI::RemoveAllStyle(bool bShared) {
        if (bShared) {
			...
			base::String *pStyle = NULL;
			for (std::map<base::String, LPVOID>::iterator it = m_ResInfo.m_StyleHash.Begin(); it != m_ResInfo.m_StyleHash.End(); it++) {
				pStyle = static_cast<base::String *>(it->second);
				if (pStyle)
					delete pStyle;
			}
    ...
    const TImageInfo *CPaintManagerUI::GetImageString(LPCTSTR pStrImage, LPCTSTR pStrModify) {
        base::String sImageName = pStrImage;
        base::String sImageResType = _T("");
        DWORD dwMask = 0;
        base::String sItem;
        base::String sValue;
        LPTSTR pstr = NULL;
            ...
            while( *pStrImage != _T('\0') ) {
                ...
                if( !sValue.IsEmpty() ) {
                    if( sItem == _T("file") || sItem == _T("res") ) {
                        ...
                        if( sValue[0] == _T('#')) dwMask = _tcstoul(sValue.GetDataPointer() + 1, &pstr, 16);
                        else dwMask = _tcstoul(sValue.GetDataPointer(), &pstr, 16);
                    }
        ...
        return GetImageEx(sImageName.GetDataPointer(), sImageResType.GetDataPointer(), dwMask);
    }
