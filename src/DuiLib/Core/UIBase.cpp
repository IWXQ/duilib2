...
namespace DuiLib {
    ...
    static const DUI_MSGMAP_ENTRY *DuiFindMessageEntry(const DUI_MSGMAP_ENTRY *lpEntry, TNotifyUI &msg ) {
        base::String sMsgType = msg.sType;
        base::String sCtrlName = msg.pSender->GetName();
    ...
    bool CNotifyPump::AddVirtualWnd(base::String strName, CNotifyPump *pObject) {
        if( m_VirtualWndMap.Find(strName.GetDataPointer()) == NULL ) {
            m_VirtualWndMap.Insert(strName.GetDataPointer(), (LPVOID)pObject);
            return true;
        }
    ...
    bool CNotifyPump::RemoveVirtualWnd(base::String strName) {
        if( m_VirtualWndMap.Find(strName.GetDataPointer()) != NULL ) {
            m_VirtualWndMap.Remove(strName.GetDataPointer());
            return true;
        }
    ...
    void CNotifyPump::NotifyPump(TNotifyUI &msg) {
        ...
        if( !msg.sVirtualWnd.IsEmpty() ) {
            for (std::map<base::String, LPVOID>::iterator it = m_VirtualWndMap.Begin(); it != m_VirtualWndMap.End(); it++) {
                if (_tcsicmp(it->first.GetDataPointer(), msg.sVirtualWnd.GetDataPointer()) == 0) {
