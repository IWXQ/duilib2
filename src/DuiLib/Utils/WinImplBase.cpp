...
namespace DuiLib {
    ...
    BOOL WindowImplBase::IsInStaticControl(CControlUI *pControl) {
        ...
        base::String strClassName = pControl->GetClass();

        std::vector<base::String> vctStaticName;
    ...
    LRESULT WindowImplBase::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL &bHandled) {
        ...
        base::String sSkinType = GetSkinType();

        if (sSkinType.IsEmpty()) {
            pRoot = builder.Create(GetSkinFile().GetDataPointer(), (UINT)0, this, &m_PaintManager);
        } else {
            STRINGorID xml(_ttoi(GetSkinFile().GetDataPointer()));
            pRoot = builder.Create(xml, sSkinType.GetDataPointer(), this, &m_PaintManager);
        }

        if (pRoot == NULL) {
            base::String sError = _T("Load Resource XML Failed£º");
            sError += GetSkinFile();
            MessageBox(NULL, sError.GetDataPointer(), _T("DuiLib"), MB_OK | MB_ICONERROR);
            return 0;
        }
