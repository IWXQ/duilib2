...
namespace DuiLib {
    ...
    LPCTSTR CButtonUI::GetHotForeImage() {
        return m_sHotForeImage.GetDataPointer();
    }
	...
	LPCTSTR CButtonUI::GetPushedForeImage() {
		return m_sPushedForeImage.GetDataPointer();
	}
	...
	base::String CButtonUI::GetFocusedForeImage() const {
	...
	base::String CButtonUI::GetDisabledForeImage() const {
    ...
    LPCTSTR CButtonUI::GetBindTabLayoutName() {
        return m_sBindTabLayoutName.GetDataPointer();
    }
    ...
    void CButtonUI::PaintText(HDC hDC) {
        ...
        if( m_bShowHtml )
            CRenderEngine::DrawHtmlText(hDC, m_pManager, rc, sText.GetDataPointer(), clrColor, NULL, NULL, nLinks, iFont, m_uTextStyle);
        else
            CRenderEngine::DrawText(hDC, m_pManager, rc, sText.GetDataPointer(), clrColor, iFont, m_uTextStyle);
    }
    ...
    void CButtonUI::PaintStatusImage(HDC hDC) {
        ...
        if( (m_uButtonState & UISTATE_DISABLED) != 0 ) {
            if( !m_sDisabledImage.IsEmpty() ) {
                if( !DrawImage(hDC, (LPCTSTR)m_sDisabledImage.GetDataPointer()) ) {}
                else return;
            }
        } else if( (m_uButtonState & UISTATE_PUSHED) != 0 ) {
    ...
    void CButtonUI::PaintForeImage(HDC hDC) {
		...
		if ((m_uButtonState & UISTATE_DISABLED) != 0) {
			if (!m_sDisabledForeImage.IsEmpty()) {
				if (DrawImage(hDC, (LPCTSTR)m_sDisabledForeImage.GetDataPointer()))
					return;
			}
		}
        else if( (m_uButtonState & UISTATE_PUSHED) != 0 ) {
            if( !m_sPushedForeImage.IsEmpty() ) {
                if(DrawImage(hDC, (LPCTSTR)m_sPushedForeImage.GetDataPointer()) )
					return;
            }
        } else if( (m_uButtonState & UISTATE_HOT) != 0 ) {
            if( !m_sHotForeImage.IsEmpty() ) {
                if(DrawImage(hDC, (LPCTSTR)m_sHotForeImage.GetDataPointer()) )
					return;
            }
        }
		else if ((m_uButtonState & UISTATE_FOCUSED) != 0) {
			if (!m_sFocusedForeImage.IsEmpty()) {
				if (DrawImage(hDC, (LPCTSTR)m_sFocusedForeImage.GetDataPointer()))
					return;
			}
		}

        if(!m_sForeImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sForeImage.GetDataPointer()) ) {
			}
        }
    }
}