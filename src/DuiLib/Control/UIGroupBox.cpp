...
namespace DuiLib {
    ...
    void CGroupBoxUI::PaintText(HDC hDC) {
        ...
        CRenderEngine::DrawText(hDC, m_pManager, rcText, sText.GetDataPointer(), dwTextColor, m_iFont, m_uTextStyle);
    }
    ...
    SIZE CGroupBoxUI::CalcTextSize(SIZE szAvailable) {
		...
		base::String strText = GetText();

        CRenderEngine::DrawText(m_pManager->GetPaintDC(), m_pManager, rcText, strText.GetDataPointer(), m_dwTextColor, m_iFont, DT_CALCRECT | m_uTextStyle);
