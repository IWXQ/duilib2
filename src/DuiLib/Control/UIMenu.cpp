...
namespace DuiLib {
    ...
    void CMenuElementUI::DrawItemExpland(HDC hDC, const RECT &rcItem) {
        if (m_bShowExplandIcon) {
            ...
            const TDrawInfo *pDrawInfo = GetManager()->GetDrawInfo((LPCTSTR)strExplandIcon.GetDataPointer(), NULL);
            const TImageInfo *pImageInfo = GetManager()->GetImageEx(pDrawInfo->sImageName.GetDataPointer(), NULL, 0);
            ...
            base::String pStrImage;
            pStrImage.Format(_T("dest='%d,%d,%d,%d'"), rcDest.left, rcDest.top, rcDest.right, rcDest.bottom);
            DrawImage(hDC, strExplandIcon.GetDataPointer(), pStrImage.GetDataPointer());
        }
    }
    ...
    void CMenuElementUI::DrawItemText(HDC hDC, const RECT &rcItem) {
        ...
        if( pInfo->bShowHtml )
            CRenderEngine::DrawHtmlText(hDC, m_pManager, rcText, sText.GetDataPointer(), iTextColor, NULL, NULL, nLinks, pInfo->nFont, DT_SINGLELINE | pInfo->uTextStyle);
        else
            CRenderEngine::DrawText(hDC, m_pManager, rcText, sText.GetDataPointer(), iTextColor, pInfo->nFont, DT_SINGLELINE | pInfo->uTextStyle);
    }
    ...
    SIZE CMenuElementUI::EstimateSize(SIZE szAvailable) {
        ...
        if(cXY.cy == 0) {
            ...
            if( pInfo->bShowHtml ) {
                int nLinks = 0;
                CRenderEngine::DrawHtmlText(m_pManager->GetPaintDC(), m_pManager, rcText, sText.GetDataPointer(), iTextColor, NULL, NULL, nLinks, pInfo->nFont, DT_CALCRECT | pInfo->uTextStyle);
            } else {
                CRenderEngine::DrawText(m_pManager->GetPaintDC(), m_pManager, rcText, sText.GetDataPointer(), iTextColor, pInfo->nFont, DT_CALCRECT | pInfo->uTextStyle);
            }
                ...
                if( hasSubMenu ) {
                    ...
                    if (IsWindow(GetManager()->GetPaintWindow()) && !isClosing) {
                        if (CMenuWnd::GetGlobalContextMenuObserver().GetNotifyRecvPaintManager() != NULL) {
                            ...
                            lstrcpy(pMenuCmd->szName, GetName().GetDataPointer());
                            lstrcpy(pMenuCmd->szUserData, GetUserData().GetDataPointer());
                            lstrcpy(pMenuCmd->szText, GetText().GetDataPointer());
    ...
    void CMenuElementUI::SetChecked(bool bCheck/* = true*/) {
		base::String strName = GetName();
        SetItemInfo(strName.GetDataPointer(), &bCheck);
    }
    ...
    bool CMenuElementUI::GetChecked() const {
        ...
        if (mCheckInfos) {
            MenuItemInfo *pItemInfo = (MenuItemInfo *)mCheckInfos->Find(strName.GetDataPointer());
    ...
    void CMenuElementUI::SetMenuEnabled(bool bEnabled /*= true*/) {
		base::String strName = GetName();
        SetItemInfo(strName.GetDataPointer(), NULL, &bEnabled);
    }
    ...
    void CMenuElementUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue) {
        if( _tcsicmp(pstrName, _T("icon")) == 0) {
            ...
            if (mCheckInfos) {
				for (std::map<base::String, LPVOID>::iterator it = mCheckInfos->Begin(); it != mCheckInfos->End(); it++) {
					MenuItemInfo *itemInfo = (MenuItemInfo *)(it->second);
					if (itemInfo) {
						base::String strName = GetName();
						if (lstrcmpi(itemInfo->szName, strName.GetDataPointer()) == 0) {
							bFind = true;
							break;
						}
					}
