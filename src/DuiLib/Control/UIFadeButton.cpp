    ...
    void CFadeButtonUI::PaintStatusImage(HDC hDC) {
        ...
        if( !m_sNormalImage.IsEmpty() ) {
            if( IsAnimationRunning(FADE_IN_ID) || IsAnimationRunning(FADE_OUT_ID)) {
                ...
                base::String sFadeOut, sFadeIn;
                sFadeOut.Format(_T("fade='%d'"), 255 - m_bFadeAlpha);
                sFadeIn.Format(_T("fade='%d'"), m_bFadeAlpha);

                if( !DrawImage(hDC, (LPCTSTR)m_sOldImage.GetDataPointer(), sFadeOut.GetDataPointer()) ) {}

                if( !DrawImage(hDC, (LPCTSTR)m_sNewImage.GetDataPointer(), sFadeIn.GetDataPointer()) ) {}
                ...
                if( !DrawImage(hDC, (LPCTSTR)m_sLastImage.GetDataPointer()) ) {}
