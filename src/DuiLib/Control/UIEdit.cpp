...
namespace DuiLib {
    ...
    void CEditWnd::Init(CEditUI *pOwner) {
        ...
        Edit_SetText(m_hWnd, m_pOwner->GetText().GetDataPointer());
    ...
    void CEditUI::SetText(LPCTSTR pstrText) {
        ...
        if( m_pWindow != NULL ) Edit_SetText(*m_pWindow, m_sText.GetDataPointer());
    ...
    LPCTSTR CEditUI::GetTipValue() {
        if (!IsResourceText()) return m_sTipValue.GetDataPointer();

        return CResourceManager::GetInstance()->GetText(m_sTipValue.GetDataPointer()).GetDataPointer();
    }
    ...
    void CEditUI::PaintText(HDC hDC) {
        ...
        base::String sDrawText = GetText();
        base::String sTipValue = GetTipValue();

        if(sDrawText == sTipValue || sDrawText == _T("")) {
            mCurTextColor = m_dwTipValueColor;
            sDrawText = sTipValue;
        } else {
            base::String sTemp = sDrawText;

            if( m_bPasswordMode ) {
                sDrawText.Empty();
                LPCTSTR pStr = sTemp.GetDataPointer();
        ...
        if( IsEnabled() ) {
            CRenderEngine::DrawText(hDC, m_pManager, rc, sDrawText.GetDataPointer(), mCurTextColor, m_iFont, DT_SINGLELINE | m_uTextStyle);
        } else {
            CRenderEngine::DrawText(hDC, m_pManager, rc, sDrawText.GetDataPointer(), m_dwDisabledTextColor, m_iFont, DT_SINGLELINE | m_uTextStyle);
        }
    }
}
