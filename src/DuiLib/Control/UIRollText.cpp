...
namespace DuiLib {
    ...
    void CRollTextUI::PaintText(HDC hDC) {
        ...
        if( m_bShowHtml ) {
            int nLinks = 0;
            CRenderEngine::DrawHtmlText(hDC, m_pManager, rc, sText.GetDataPointer(), dwTextColor, NULL, NULL, nLinks, m_iFont, uTextStyle);
        } else {
            CRenderEngine::DrawText(hDC, m_pManager, rc, sText.GetDataPointer(), dwTextColor, m_iFont, uTextStyle);
        }
