...
namespace DuiLib {
    ...
    LPCTSTR CGifAnimUI::GetGifImage() {
        return m_sGifImage.GetDataPointer();
    }
    ...
    void CGifAnimUI::DrawFrame( HDC hDC ) {
        ...
        Gdiplus::Status status = graphics.DrawImage( m_pGifImage, m_rcItem.left, m_rcItem.top, m_rcItem.right - m_rcItem.left, m_rcItem.bottom - m_rcItem.top );
        ASSERT(status == 0);
