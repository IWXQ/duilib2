...
namespace DuiLib {
    ...
    LPCTSTR CListUI::GetItemBkImage() const {
        return m_ListInfo.sBkImage.GetDataPointer();
    }
    ...
    LPCTSTR CListUI::GetSelectedItemImage() const {
        return m_ListInfo.sSelectedImage.GetDataPointer();
    }
    ...
    LPCTSTR CListUI::GetHotItemImage() const {
        return m_ListInfo.sHotImage.GetDataPointer();
    }
    ...
    LPCTSTR CListUI::GetDisabledItemImage() const {
        return m_ListInfo.sDisabledImage.GetDataPointer();
    }
    ...
    LPCTSTR CListHeaderItemUI::GetNormalImage() const {
        return m_sNormalImage.GetDataPointer();
    }
    ...
    LPCTSTR CListHeaderItemUI::GetHotImage() const {
        return m_sHotImage.GetDataPointer();
    }
    ...
    LPCTSTR CListHeaderItemUI::GetPushedImage() const {
        return m_sPushedImage.GetDataPointer();
    }
    ...
    LPCTSTR CListHeaderItemUI::GetFocusedImage() const {
        return m_sFocusedImage.GetDataPointer();
    }
    ...
    LPCTSTR CListHeaderItemUI::GetSepImage() const {
        return m_sSepImage.GetDataPointer();
    }
    ...
    void CListHeaderItemUI::PaintStatusImage(HDC hDC) {
        ...
        if( (m_uButtonState & UISTATE_PUSHED) != 0 ) {
            if( m_sPushedImage.IsEmpty() && !m_sNormalImage.IsEmpty() ) DrawImage(hDC, (LPCTSTR)m_sNormalImage.GetDataPointer());

            if( !DrawImage(hDC, (LPCTSTR)m_sPushedImage.GetDataPointer()) ) {}
        } else if( (m_uButtonState & UISTATE_HOT) != 0 ) {
            if( m_sHotImage.IsEmpty() && !m_sNormalImage.IsEmpty() ) DrawImage(hDC, (LPCTSTR)m_sNormalImage.GetDataPointer());

            if( !DrawImage(hDC, (LPCTSTR)m_sHotImage.GetDataPointer()) ) {}
        } else if( (m_uButtonState & UISTATE_FOCUSED) != 0 ) {
            if( m_sFocusedImage.IsEmpty() && !m_sNormalImage.IsEmpty() ) DrawImage(hDC, (LPCTSTR)m_sNormalImage.GetDataPointer());

            if( !DrawImage(hDC, (LPCTSTR)m_sFocusedImage.GetDataPointer()) ) {}
        } else {
            if( !m_sNormalImage.IsEmpty() ) {
                if( !DrawImage(hDC, (LPCTSTR)m_sNormalImage.GetDataPointer()) ) {}
            }
        }
        ...
        if( !m_sSepImage.IsEmpty() ) {
            ...
            if( !DrawImage(hDC, (LPCTSTR)m_sSepImage.GetDataPointer(), (LPCTSTR)m_sSepImageModify.GetDataPointer()) ) {
            }
        }
    }
    ...
    void CListHeaderItemUI::PaintText(HDC hDC) {
        ...
        if( m_bShowHtml )
            CRenderEngine::DrawHtmlText(hDC, m_pManager, rcText, sText.GetDataPointer(), m_dwTextColor, NULL, NULL, nLinks, m_iFont, m_uTextStyle);
        else
            CRenderEngine::DrawText(hDC, m_pManager, rcText, sText.GetDataPointer(), m_dwTextColor, m_iFont, m_uTextStyle);
    }
    ...
    void CListElementUI::DrawItemBk(HDC hDC, const RECT &rcItem) {
        ...
        if( !IsEnabled() ) {
            if( !pInfo->sDisabledImage.IsEmpty() ) {
                if( !DrawImage(hDC, (LPCTSTR)pInfo->sDisabledImage.GetDataPointer()) ) {}
                else return;
            }
        }
        ...
        if( IsSelected() ) {
            if( !pInfo->sSelectedImage.IsEmpty() ) {
                if( !DrawImage(hDC, (LPCTSTR)pInfo->sSelectedImage.GetDataPointer()) ) {}
                else return;
            }
        }
        ...
        if( (m_uButtonState & UISTATE_HOT) != 0 ) {
            if( !pInfo->sHotImage.IsEmpty() ) {
                if( !DrawImage(hDC, (LPCTSTR)pInfo->sHotImage.GetDataPointer()) ) {}
                else return;
            }
        }
        ...
        if( !m_sBkImage.IsEmpty() ) {
            if( !pInfo->bAlternateBk || m_iIndex % 2 == 0 ) {
                if( !DrawImage(hDC, (LPCTSTR)m_sBkImage.GetDataPointer()) ) {}
            }
        }

        if( m_sBkImage.IsEmpty() ) {
            if( !pInfo->sBkImage.IsEmpty() ) {
                if( !DrawImage(hDC, (LPCTSTR)pInfo->sBkImage.GetDataPointer()) ) {
                }
                else {
                    return;
                }
    ...
    SIZE CListLabelElementUI::EstimateSize(SIZE szAvailable) {
        ...
        if (m_pManager) {
            ...
            if (cXY.cx == 0) {
                ...
                if (pInfo->bShowHtml) {
                    int nLinks = 0;
                    CRenderEngine::DrawHtmlText(m_pManager->GetPaintDC(), m_pManager, rcText, sText.GetDataPointer(), 0, NULL, NULL, nLinks, pInfo->nFont, DT_SINGLELINE | DT_CALCRECT | pInfo->uTextStyle & ~DT_RIGHT & ~DT_CENTER);
                }
                else {
                    CRenderEngine::DrawText(m_pManager->GetPaintDC(), m_pManager, rcText, sText.GetDataPointer(), 0, pInfo->nFont, DT_SINGLELINE | DT_CALCRECT | pInfo->uTextStyle & ~DT_RIGHT & ~DT_CENTER);
                }
    ...
    void CListLabelElementUI::DrawItemText(HDC hDC, const RECT &rcItem) {
        ...
        if( pInfo->bShowHtml )
            CRenderEngine::DrawHtmlText(hDC, m_pManager, rcText, sText.GetDataPointer(), iTextColor, NULL, NULL, nLinks, pInfo->nFont, pInfo->uTextStyle);
        else
            CRenderEngine::DrawText(hDC, m_pManager, rcText, sText.GetDataPointer(), iTextColor, pInfo->nFont, pInfo->uTextStyle);
    }
    ...
    CListTextElementUI::~CListTextElementUI() {
        ...
        for( int it = 0; it < m_aTexts.GetSize(); it++ ) {
            pText = static_cast<base::String *>(m_aTexts[it]);
    ...
    LPCTSTR CListTextElementUI::GetText(int iIndex) const {
        base::String *pText = static_cast<base::String *>(m_aTexts.GetAt(iIndex));

        if( pText ) {
            if (!IsResourceText())
                return pText->GetDataPointer();

            return CResourceManager::GetInstance()->GetText(pText->GetDataPointer()).GetDataPointer();
        }
    ...
    void CListTextElementUI::SetText(int iIndex, LPCTSTR pstrText) {
        ...
        base::String *pText = static_cast<base::String *>(m_aTexts[iIndex]);
        ...
        m_aTexts.SetAt(iIndex, new base::String(pstrText));
    ...
    base::String *CListTextElementUI::GetLinkContent(int iIndex) {
    ...
    void CListTextElementUI::DrawItemText(HDC hDC, const RECT &rcItem) {
        ...
        for( int i = 0; i < pInfo->nColumns; i++ ) {
            ...
            if( pInfo->bShowHtml )
                CRenderEngine::DrawHtmlText(hDC, m_pManager, rcItem2, strText.GetDataPointer(), iTextColor, &m_rcLinks[m_nLinks], &m_sLinks[m_nLinks], nLinks, pInfo->nFont, pInfo->uTextStyle);
            else
                CRenderEngine::DrawText(hDC, m_pManager, rcItem2, strText.GetDataPointer(), iTextColor, pInfo->nFont, pInfo->uTextStyle);
