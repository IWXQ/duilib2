...
namespace DuiLib {
    ...
    LPCTSTR CSliderUI::GetThumbImage() const {
        return m_sThumbImage.GetDataPointer();
    }
	...
	LPCTSTR CSliderUI::GetThumbFocusedImage() const {
		return m_sThumbFocusedImage.GetDataPointer();
	}
	...
	void CSliderUI::PaintForeImage(HDC hDC) {
        ...
        if( (m_uButtonState & UISTATE_CAPTURED) != 0 ) {
			...
			if (!m_sThumbFocusedImage.IsEmpty()) {
				...
				if (DrawImage(hDC, (LPCTSTR)m_sThumbFocusedImage.GetDataPointer(), (LPCTSTR)m_sImageModify.GetDataPointer()))
					return;
			}
		}
        ...
        if( !m_sThumbImage.IsEmpty() ) {
            ...
            if(DrawImage(hDC, (LPCTSTR)m_sThumbImage.GetDataPointer(), (LPCTSTR)m_sImageModify.GetDataPointer()))
				return;
        }
    }
