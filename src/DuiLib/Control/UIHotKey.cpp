...
namespace DuiLib {
    ...
    LRESULT CHotKeyWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam) {
        ...
        if( uMsg == WM_KILLFOCUS ) lRes = OnKillFocus(uMsg, wParam, lParam, bHandled);
            ...
            base::String strText = GetHotKeyName();
            ::RECT rect;
            ...
            ::SIZE size = { 0 };
            ::GetTextExtentPoint32(hDC, strText.GetDataPointer(), strText.GetLength(), &size) ;
            ::DrawText(hDC, strText.GetDataPointer(), -1, &rect, DT_LEFT | DT_SINGLELINE | DT_END_ELLIPSIS | DT_NOPREFIX);
    ...
    base::String CHotKeyWnd::GetKeyName(UINT vk, BOOL fExtended) {
        ...
        return base::String(szStr);
    }


    base::String CHotKeyWnd::GetHotKeyName() {
        ASSERT(::IsWindow(m_hWnd));

        base::String strKeyName;
        WORD wCode = 0;
        WORD wModifiers = 0;
    ...
    void CHotKeyUI::PaintText(HDC hDC) {
        ...
        CRenderEngine::DrawText(hDC, m_pManager, rc, sText.GetDataPointer(), dwTextColor, m_iFont, DT_SINGLELINE | m_uTextStyle);
    }
