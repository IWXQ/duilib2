...
namespace DuiLib {
    ...
    COptionUI::~COptionUI() {
        if( !m_sGroupName.IsEmpty() && m_pManager ) m_pManager->RemoveOptionGroup(m_sGroupName.GetDataPointer(), this);
    }
    ...
    void COptionUI::SetManager(CPaintManagerUI *pManager, CControlUI *pParent, bool bInit) {
        ...
        if( bInit && !m_sGroupName.IsEmpty() ) {
            if (m_pManager) m_pManager->AddOptionGroup(m_sGroupName.GetDataPointer(), this);
        }
    }

    LPCTSTR COptionUI::GetGroup() const {
        return m_sGroupName.GetDataPointer();
    }
    ...
    void COptionUI::SetGroup(LPCTSTR pStrGroupName) {
        ...
        if( !m_sGroupName.IsEmpty() ) {
            ...
            if (m_pManager) m_pManager->RemoveOptionGroup(m_sGroupName.GetDataPointer(), this);
        }
    ...
    void COptionUI::Selected(bool bSelected, bool bMsg/* = true*/) {
        ...
        if( m_pManager != NULL ) {
            if( !m_sGroupName.IsEmpty() ) {
                if( m_bSelected ) {
                    CStdPtrArray *aOptionGroup = m_pManager->GetOptionGroup(m_sGroupName.GetDataPointer());
    ...
    LPCTSTR COptionUI::GetSelectedImage() {
        return m_sSelectedImage.GetDataPointer();
    }
    ...
    LPCTSTR COptionUI::GetSelectedHotImage() {
        return m_sSelectedHotImage.GetDataPointer();
    }
    ...
    LPCTSTR COptionUI::GetSelectedPushedImage() {
        return m_sSelectedPushedImage.GetDataPointer();
    }
    ...
    LPCTSTR COptionUI::GetSelectedForedImage() {
        return m_sSelectedForeImage.GetDataPointer();
    }
    ...
    LPCTSTR COptionUI::GetSelectedStateImage() {
        return m_sSelectedStateImage.GetDataPointer();
    }
    ...
    void COptionUI::PaintStatusImage(HDC hDC) {
        if(IsSelected()) {
            if(!m_sSelectedStateImage.IsEmpty() && m_nSelectedStateCount > 0) {
                const TDrawInfo *info = m_pManager->GetDrawInfo(m_sSelectedStateImage.GetDataPointer(), _T(""));
                const TImageInfo *pImage = m_pManager->GetImageEx(info->sImageName.GetDataPointer(), info->sResType.GetDataPointer(), info->dwMask, info->bHSL);

                if (!pImage) {
                    pImage = m_pManager->GetImageEx(info->sImageName.GetDataPointer(), info->sResType.GetDataPointer(), info->dwMask, info->bHSL);
                }
                    ...
                    if( szImage.cx > 0 && szImage.cy > 0 ) {
                        ...
                        if(m_nSelectedStateCount > 0) {
                            ...
                            m_sSelectedImage.Format(_T("res='%s' restype='%s' dest='%d,%d,%d,%d' source='%d,%d,%d,%d'"), 
                                info->sImageName.GetDataPointer(), 
                                info->sResType.GetDataPointer(), 
                                info->rcDest.left,
                                info->rcDest.top, 
                                info->rcDest.right,
                                info->rcDest.bottom,
            ...
            if( (m_uButtonState & UISTATE_PUSHED) != 0 && !m_sSelectedPushedImage.IsEmpty()) {
                if( !DrawImage(hDC, (LPCTSTR)m_sSelectedPushedImage.GetDataPointer()) ) {
                }
                else 
                    return;
            } else if( (m_uButtonState & UISTATE_HOT) != 0 && !m_sSelectedHotImage.IsEmpty()) {
                if( !DrawImage(hDC, (LPCTSTR)m_sSelectedHotImage.GetDataPointer()) ) {
                }
                else 
                    return;
            }
            ...
            if( !m_sSelectedImage.IsEmpty() ) {
                if( !DrawImage(hDC, (LPCTSTR)m_sSelectedImage.GetDataPointer()) ) {
                }
            }
        } 
        else {
    ...
    void COptionUI::PaintForeImage(HDC hDC) {
        if(IsSelected()) {
            if( !m_sSelectedForeImage.IsEmpty() ) {
                if( !DrawImage(hDC, (LPCTSTR)m_sSelectedForeImage.GetDataPointer()) ) {
                }
                else 
                    return;
            }
    ...
    void COptionUI::PaintText(HDC hDC) {
        if( (m_uButtonState & UISTATE_SELECTED) != 0 ) {
            ...
            if( m_bShowHtml )
                CRenderEngine::DrawHtmlText(hDC, m_pManager, rc, sText.GetDataPointer(), IsEnabled() ? m_dwTextColor : m_dwDisabledTextColor, NULL, NULL, nLinks, iFont, m_uTextStyle);
            else
                CRenderEngine::DrawText(hDC, m_pManager, rc, sText.GetDataPointer(), IsEnabled() ? m_dwTextColor : m_dwDisabledTextColor, iFont, m_uTextStyle);
