...
namespace DuiLib {
    class CComboBoxWnd : public CWindowWnd, public INotifyUI {
      public:
        ...
        base::String GetText();
        void SetText(const base::String &str);
      public:
    ...
    void CComboBoxWnd::Notify(TNotifyUI &msg) {
        if (msg.sType == DUI_MSGTYPE_WINDOWINIT) {
            ...
            base::String sName = msg.pSender->GetName();
    ...
    base::String CComboBoxWnd::GetText() {
        if (m_pList) {
    ...
    void CComboBoxWnd::SetText(const base::String &str) {
        if(m_pOwner)
            m_pOwner->SetText(str.GetDataPointer());
    }
    ...
    LRESULT CComboBoxWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam) {
        if( uMsg == WM_CREATE ) {
            ...
            m_pList->ApplyAttributeList(m_pOwner->GetDropBoxAttributeList().GetDataPointer());
    ...
    base::String CComboBoxUI::GetText() const {
        if (m_pWindow)
    ...
    base::String CComboBoxUI::GetDropBoxAttributeList() {
    ...
    LPCTSTR CComboBoxUI::GetDisabledImage() const {
        return m_sDisabledImage.GetDataPointer();
    }
    ...
    LPCTSTR CComboBoxUI::GetArrowNormalImage() const {
        return m_sArrowNormalImage.GetDataPointer();
    }
    ...
    LPCTSTR CComboBoxUI::GetArrowHotImage() const {
        return m_sArrowHotImage.GetDataPointer();
    }
    ...
    LPCTSTR CComboBoxUI::GetArrowPushedImage() const {
        return m_sArrowPushedImage.GetDataPointer();
    }
    ...
    LPCTSTR CComboBoxUI::GetArrowFocusedImage() const {
        return m_sArrowFocusedImage.GetDataPointer();
    }
    ...
    LPCTSTR CComboBoxUI::GetArrowDisabledImage() const {
        return m_sArrowDisabledImage.GetDataPointer();
    }
    ...
    void CComboBoxUI::PaintStatusImage(HDC hDC) {
        ...
        if( (m_uButtonState & UISTATE_DISABLED) != 0 ) {
            if( !m_sDisabledImage.IsEmpty() ) {
                bDrawSuccessed = DrawImage(hDC, (LPCTSTR)m_sDisabledImage.GetDataPointer());
            }
        } 
        else if( (m_uButtonState & UISTATE_PUSHED) != 0 ) {
            if( !m_sPushedImage.IsEmpty() ) {
                bDrawSuccessed = DrawImage(hDC, (LPCTSTR)m_sPushedImage.GetDataPointer());
            }
        } 
        else if( (m_uButtonState & UISTATE_HOT) != 0 ) {
            if( !m_sHotImage.IsEmpty() ) {
                bDrawSuccessed = DrawImage(hDC, (LPCTSTR)m_sHotImage.GetDataPointer());
            }
        } 
        else if( (m_uButtonState & UISTATE_FOCUSED) != 0 ) {
            if( !m_sFocusedImage.IsEmpty() ) {
                bDrawSuccessed = DrawImage(hDC, (LPCTSTR)m_sFocusedImage.GetDataPointer());
            }
        }
        ...
        if ((m_uButtonState & UISTATE_DISABLED) != 0) {
            if (!m_sArrowDisabledImage.IsEmpty()) {
                if (!DrawImage(hDC, (LPCTSTR)m_sArrowDisabledImage.GetDataPointer())) {}
                else return;
            }
        }
        else if ((m_uButtonState & UISTATE_PUSHED) != 0) {
            if (!m_sArrowPushedImage.IsEmpty()) {
                if (!DrawImage(hDC, (LPCTSTR)m_sArrowPushedImage.GetDataPointer())) {}
                else return;
            }
        }
        else if ((m_uButtonState & UISTATE_HOT) != 0) {
            if (!m_sArrowHotImage.IsEmpty()) {
                if (!DrawImage(hDC, (LPCTSTR)m_sArrowHotImage.GetDataPointer())) {}
                else return;
            }
        }
        else if ((m_uButtonState & UISTATE_FOCUSED) != 0) {
            if (!m_sArrowFocusedImage.IsEmpty()) {
                if (!DrawImage(hDC, (LPCTSTR)m_sArrowFocusedImage.GetDataPointer())) {}
                else return;
            }
        }

        if (!m_sArrowNormalImage.IsEmpty()) {
            if (!DrawImage(hDC, (LPCTSTR)m_sArrowNormalImage.GetDataPointer())) {}
        }
    }
    ...
    void CComboBoxUI::PaintText(HDC hDC) {
        ...
        if( IsEnabled() ) {
            if( m_bShowHtml )
                CRenderEngine::DrawHtmlText(hDC, m_pManager, rc, sText.GetDataPointer(), m_dwTextColor, NULL, NULL, nLinks, m_iFont, m_uTextStyle);
            else
                CRenderEngine::DrawText(hDC, m_pManager, rc, sText.GetDataPointer(), m_dwTextColor, m_iFont, m_uTextStyle);
        } 
        else {
            if( m_bShowHtml )
                CRenderEngine::DrawHtmlText(hDC, m_pManager, rc, sText.GetDataPointer(), m_dwDisabledTextColor, NULL, NULL, nLinks, m_iFont, m_uTextStyle);
            else
                CRenderEngine::DrawText(hDC, m_pManager, rc, sText.GetDataPointer(), m_dwDisabledTextColor, m_iFont, m_uTextStyle);
        }
    }
