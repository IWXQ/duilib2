	...
	for (int i = 0; i < numOfItem; i++)
	{
		...
		if (m_bShowText)
		{
			...
			if (::IntersectRect(&m_rcPaint, &rcText, &m_rcItem))
			{
				...
				if( m_bShowHtml )
				{
					CRenderEngine::DrawHtmlText(hDC, m_pManager, rcText, m_items[i].name.GetDataPointer(), clrColor, NULL, NULL, nLinks, m_iFont, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
				}
				else
				{
					CRenderEngine::DrawText(hDC, m_pManager, rcText, m_items[i].name.GetDataPointer(), clrColor, m_iFont, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
				}
			}
...
void CChartViewUI::DoPaintHistogram(HDC hDC, const RECT& rcPaint)
{
	...
	if (!m_sShadowImage.IsEmpty())
	{
		...
		if (!CRenderEngine::DrawImageString(hDC, m_pManager, rcShadow, m_rcPaint, m_sShadowImage.GetDataPointer(), NULL))
		{
	...
	for (i = 0; i < numOfItem; i++)
	{
		...
		if (!m_sPillarImage.IsEmpty())
		{
			...
			if (!CRenderEngine::DrawImageString(hDC, m_pManager, rcPillar, m_rcPaint, m_sPillarImage.GetDataPointer(), NULL))
			{
		...
		if (m_bShowText)
		{
			...
			if( m_bShowHtml )
			{
				CRenderEngine::DrawHtmlText(hDC, m_pManager, rcText, m_items[i].name.GetDataPointer(), clrColor, NULL, NULL, nLinks, m_iFont, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
			}
			else
			{
				CRenderEngine::DrawText(hDC, m_pManager, rcText, m_items[i].name.GetDataPointer(), clrColor, m_iFont, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
			}
		}
