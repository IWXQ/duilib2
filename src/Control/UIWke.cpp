...
#define WUM_REFRESH_TIMER (1)
    ...
    void CWkeWnd::SetUrl(const CDuiString &strUrl) {
        m_strUrl = strUrl;

        if (m_pWebView && m_strUrl.GetLength() > 0) {
            wkeLoadURL(m_pWebView, UnicodeToUtf8(m_strUrl.GetData()).c_str());
        }
    }

    void CWkeWnd::SetBkImage(const CDuiString &strBkImage) {
    ...
    void CWkeWnd::ShowDevTools() {
        if (m_pWebView && wkeShowDevtools) {
            std::wstring strInspectorPath = CPaintManagerUI::GetInstancePath();
    ...
    static void jsValue2VARINAT(const jsExecState &es, const jsValue &retval, VARIANT *pvRet) {
        ...
        if (jsIsString(retval)) {
            V_VT(pvRet) = VT_BSTR;
            V_BSTR(pvRet) = SysAllocString(Utf8ToUnicode(jsToString(es, retval)).c_str());
    ...
    VARIANT CWkeWnd::CallJavascriptFunction(const std::string &strFuncName, const std::vector<VARIANT> &vars) {
        ...
        for (size_t i = 0; i < vars.size(); i++) {
            switch (vars[i].vt) {
                case VT_BSTR:
                    jsArgs[i] = jsString(es, UnicodeToUtf8(vars[i].bstrVal).c_str());
                    break;
    ...
    void CWkeUI::SetURL(const CDuiString &strValue) {
        m_strUrl = strValue;
    ...
    void CWkeUI::SetBkImage(const CDuiString &strValue) {
